...
def update_patient(id):
    ...
    if request.method == 'POST':
        cursor.execute('''
            UPDATE Patients SET name=%s, age=%s, gender=%s, blood_type=%s, medical_condition=%s
            WHERE patient_id=%s
        ''', (
            request.form['name'], request.form['age'], request.form['gender'],
...
def update_doctor(id):
    ...
    if request.method == 'POST':
        cursor.execute('''
            UPDATE Doctors SET name=%s, hospital_id=%s WHERE doctor_id=%s
        ''', (request.form['name'], request.form['hospital_id'], id))
        conn.commit()
        conn.close()
        return redirect(url_for('doctors'))
    else:
        cursor.execute('SELECT doctor_id, name, hospital_id FROM Doctors WHERE doctor_id = %s', (id,))
...
def add_hospital():
    if request.method == 'POST':
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('INSERT INTO Hospitals (name) VALUES (%s)', (request.form['name'],))
        conn.commit()
        conn.close()
...
def delete_hospital(id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('DELETE FROM Hospitals WHERE hospital_id = %s', (id,))
    conn.commit()
    conn.close()
    return redirect(url_for('hospitals'))
...
def update_admission(id):
    ...
    if request.method == 'POST':
        cursor.execute('''
            UPDATE Admissions
            SET patient_id=%s, doctor_id=%s, hospital_id=%s, admission_date=%s, discharge_date=%s, room_number=%s, admission_type=%s
    ...
    cursor.execute('SELECT * FROM Admissions WHERE admission_id = %s', (id,))
...
def delete_admission(id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('DELETE FROM Admissions WHERE admission_id = %s', (id,))
    conn.commit()
    conn.close()
    return redirect(url_for('admissions'))
...
def update_billing(id):
    ...
    if request.method == 'POST':
        cursor.execute('''
            UPDATE Billing
            SET patient_id=%s, insurance_provider=%s, billing_amount=%s
            WHERE billing_id=%s
        ''', (
            request.form['patient_id'],
    ...
    cursor.execute('SELECT * FROM Billing WHERE billing_id = %s', (id,))
...
def delete_billing(id):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('DELETE FROM Billing WHERE billing_id = %s', (id,))
    conn.commit()
    conn.close()
    return redirect(url_for('billing'))
